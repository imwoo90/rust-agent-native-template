...
2. **Compile-Time Architectural Limits (`build.rs`)**:
   ...
   - **Inline Test Budget**: Keep inline unit tests (`#[cfg(test)]`) under **5,000 characters** in `src/`. Move integration tests to root `tests/`, and extract large private unit suites into submodule files (e.g. `src/<module>/tests.rs`).
